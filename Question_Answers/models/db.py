...
db.define_table('comment1',
    Field('body','text'),
    Field('author',db.auth_user,requires=IS_IN_DB(db,'auth_user.id','auth_user.first_name'),default=auth.user_id),
    Field('pid',db.Blog_post,requires=IS_IN_DB(db,'Blog_post.id','Blog_post.subject'),readable=False,writable=False))
